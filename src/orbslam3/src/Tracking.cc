...
Tracking::Tracking(System *pSys, ORBVocabulary *pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas,
    ...
    : mState(NO_IMAGES_YET), mSensor(sensor), mTrackedFr(0), mbStep(false), mbOnlyTracking(false), mbMapUpdated(false),
      ...
      mpCamera2(nullptr), mpLastKeyFrame(static_cast<KeyFrame *>(NULL)), mSettings(settings)
{
...
Sophus::SE3f Tracking::GrabImageMonocularGPS(const cv::Mat &im, const double &timestamp, const GPSPos &gps,
                                             string filename)
{
    mImGray = im;
    if (mImGray.channels() == 3)
    ...
    {
        throw std::runtime_error("IMU_MONOCULAR not implemented yet for GPS");
    }
