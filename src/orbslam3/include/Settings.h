...
class Settings
{
  public:
     ...
     */
    Settings(const std::string &configFile, const int &sensor, const std::string &outDir);
  ...
  private:
    ...
    void readAtlasFile(cv::FileStorage &fSettings);
    void readGPSTransform(cv::FileStorage &fSettings);
    ...
    std::string sAtlasFile_;

    /*
     * Root path for saving any output generated by the system
     */
    std::string sOutDir_;
     ...
     */
    Eigen::Matrix3f gpsTransformRotation_;
    Eigen::Vector3f gpsTransformTranslation_;
    double gpsTransformScale_;
